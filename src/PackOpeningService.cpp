...
PackOpenResult PackOpeningService::openPack(const Pack &pack,
                                            GameState &gameState,
...
Rarity PackOpeningService::rollRarity(const Pack &pack) {
  ...
  for (const auto &oddsPair : pack.getRarityOdds()) {
    totalWeight += max(0, oddsPair.second);
  }
  ...
  uniform_int_distribution<int> weightDistribution(1, totalWeight);
  ...
  for (int rarityIndex = 0; rarityIndex < rarityCount; ++rarityIndex) {
    ...
    rolledWeight -= max(0, oddsIterator->second);
    if (rolledWeight <= 0) {
      return rarity;
    }
  }
...
const Card *PackOpeningService::rollCard(const Pack &pack, Rarity rarity,
                                         const CardDatabase &cardDatabase) {
  vector<const Card *> candidates;
  for (const string &cardId : pack.getEligibleCardIds()) {
  ...
  uniform_int_distribution<int> cardDistribution(
      0, static_cast<int>(candidates.size()) - 1);
  return candidates[static_cast<size_t>(cardDistribution(randomEngine))];
}

bool PackOpeningService::shouldAutoSell(
    const Card &card, const AutoSellSettings &autoSellSettings) const {
  ...
  const bool rarityMatches = autoSellSettings.sellByRarity &&
                             static_cast<int>(card.getRarity()) <=
                                 static_cast<int>(autoSellSettings.maxRarity);
  const bool valueMatches = autoSellSettings.sellByValue &&
                            card.getSellValue() >= autoSellSettings.minValue &&
                            card.getSellValue() <= autoSellSettings.maxValue;
